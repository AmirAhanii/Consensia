...
# + Importance weighting (Generative Agents, Section 4.1)
STAGE1_SYSTEM = """
...
Papers with higher importance_weight carry more evidential weight.
...
def compute_importance_weight(paper: dict) -> float:
    """
    Compute a normalized importance weight per paper using citation count.
    Generative Agents, Section 4.1 — Importance Scoring.
    Log scale prevents highly-cited papers from completely dominating.
    Papers with null citations receive minimum weight 0.1.
    """
    citations = paper.get("citation_count")
    if citations is None or citations == 0:
        return 0.1
    return round(min(1.0, 0.1 + 0.9 * (math.log1p(citations) / math.log1p(100))), 3)


def add_importance_weights(papers: list) -> list:
    """
    Add importance_weight to each paper and sort descending.
    High-weight papers appear first in each chunk so they
    are never lost in the middle of the context window.
    """
    weighted = []
    for paper in papers:
        p = dict(paper)
        p["importance_weight"] = compute_importance_weight(paper)
        weighted.append(p)
    return sorted(weighted, key=lambda x: x["importance_weight"], reverse=True)
...
def generate_persona_from_raw(raw_author: dict, author_name: str, raw_filename: str) -> dict:
    ...
    # Importance weighting — Generative Agents, Section 4.1
    weighted_papers = add_importance_weights(papers)
    paper_chunks = chunk_papers(weighted_papers, CHUNK_SIZE)
