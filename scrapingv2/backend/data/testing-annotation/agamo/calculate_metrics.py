...
def calculate_scientific_scores():
    # 1. Load Ground Truth (Real Alireza)
    with open('aaghamohammadi_only_context.json', 'r', encoding='utf-8') as f:
        ground_truth_data = json.load(f)

    # 2. Load Predictions (LLM Alireza)
    with open('aaghamohammadi_api_results.json', 'r', encoding='utf-8') as f:
    ...
    for i, true_hunk in enumerate(ground_truth_data):
        ...
        true_labels_str = true_hunk.get('aaghamohammadi', '{}')
        try:
