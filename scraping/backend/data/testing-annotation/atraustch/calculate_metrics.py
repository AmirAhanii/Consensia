...
def calculate_scientific_scores():
    # 1. Load Ground Truth (Real Alex)
    with open('atrautsch_only_context.json', 'r', encoding='utf-8') as f:
        ground_truth_data = json.load(f)

    # 2. Load Predictions (LLM Alex)
    with open('atrautsch_api_results.json', 'r', encoding='utf-8') as f:
    ...
    for i, true_hunk in enumerate(ground_truth_data):
        ...
        true_labels_str = true_hunk.get('atrautsch', '{}')
        try:
